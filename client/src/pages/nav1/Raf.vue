<template>
  <div class="demo-container">
    ...
    <div class="controls">
      ...
      <button @click="clearList" :disabled="isRendering">🗑️ 清空列表</button>
    </div>
...
const renderWithoutRAF = async () => {
  ...
  const estimatedFPS = Math.max(
    0,
    Math.min(60, Math.floor(1000 / (renderTime.value / 30))),
  );
  ...
  if (renderTime.value > 100) {
    alert(
      `一次性渲染完成！耗时 ${renderTime.value}ms，页面卡顿了 ${(renderTime.value / 1000).toFixed(1)} 秒`,
    );
  }
};
...
const renderWithRAF = async () => {
  ...
  const renderChunk = () => {
    ...
    if (currentIndex < allData.length) {
      ...
      const estimatedFPS = Math.floor(
        allData.length / CHUNK_SIZE / (totalTime / 1000),
      );
      ...
      console.log(
        `RAF分片渲染完成！总耗时 ${renderTime.value}ms，分片数 ${chunkCount.value}`,
      );
    }
  };
